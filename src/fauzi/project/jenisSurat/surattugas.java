...
public class surattugas {
    String kode="", nama="", tempatLahir="", tanggalLahir="", jk="", alamat="", kades="", tahun="", tujuan="";
    String tanggal = "00-00-0000";
    
    public surattugas(String kode, String nama, String tempatLahir, String tanggalLahir, String jk, String alamat, String kades, String tahun, String tujuan, String tanggal){
        this.kode = kode;
        this.nama = nama;
    ...
    public void buat(){
        ...
        myDoc.addEle(Paragraph.withPieces(
                ...
                ParagraphPiece.with("SURAT PERINTAH TUGAS").withStyle().font(Font.TIMES_NEW_ROMAN_BOLD).fontSize("15").underline().create()
        ));
        myDoc.addEle(Paragraph.withPieces(
                ParagraphPiece.with("\t\t\t\t").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("15").create(),
                ParagraphPiece.with("Nomor : 475/"+kode+"/432.501.18/"+tahun+"").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("15").create()
        ));
        ...
        myDoc.addEle(Paragraph.withPieces(
                ...
                ParagraphPiece.with("Yang bertanda tangan di bawah ini Kepala Desa Toronan Kecamatan Pamekasan Kabupaten Pamekasan menugaskan:").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("12").create()
        ));       
        ...
        myDoc.addEle(Paragraph.withPieces(
                ParagraphPiece.with("\t").withStyle().font(Font.TIMES_NEW_ROMAN_BOLD).fontSize("12").create(),
                ParagraphPiece.with("Sebagai Petugas "+tujuan+" di Desa Toronan Kecamatan Pamekasan Kabupaten Pamekasan Tahun "+tahun+".").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("12").create()
        ));               
        //content main
        ...
        myDoc.addEle(Paragraph.withPieces(
                ParagraphPiece.with("\t").withStyle().font(Font.TIMES_NEW_ROMAN_BOLD).fontSize("12").create(),
                ParagraphPiece.with("Demikian surat tugas diberikan untuk dilaksanakan dengan penuh tanggung jawab.").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("12").create()
        ));               
