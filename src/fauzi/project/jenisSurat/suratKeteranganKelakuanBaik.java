 ...
 * @author fauzi
 */
public class suratKeteranganKelakuanBaik {
    String kode="", nama="", tempatl, tgllahir="", jk="", agama="", job="", ktp="", alamat="", kades="";    
    String tanggal = "00-00-0000";
    
    public suratKeteranganKelakuanBaik(String kode, String nama, String tempatl, String tgllahir, String jk, String agama, String job, String ktp, String alamat, String kades, String tgl){
        this.kode = kode;
        this.nama = nama;
    ...
    public void buat(){
        ...
        myDoc.addEle(Paragraph.withPieces(
                ...
                ParagraphPiece.with("SURAT KETERANGAN BERKELAKUAN BAIK").withStyle().font(Font.TIMES_NEW_ROMAN_BOLD).fontSize("15").underline().create()
        ));
        myDoc.addEle(Paragraph.withPieces(
                ParagraphPiece.with("\t\t\t\t").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("15").create(),
                ParagraphPiece.with("Nomor : 474/"+kode+"/432.501/81/2012").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("15").create()
        ));
        ...
        myDoc.addEle(Paragraph.with("\t\tTempat Tanggal Lahir\t:"+tempatl+", "+tgllahir).create());
        ...
        myDoc.addEle(Paragraph.with("\t\tNomor KTP \t\t\t:"+ktp).create());
        ...
        myDoc.addEle(Paragraph.withPieces(
                ParagraphPiece.with("\t").withStyle().font(Font.TIMES_NEW_ROMAN_BOLD).fontSize("12").create(),
                ParagraphPiece.with("Sepanjang pengetahuan kami dan selama berada di daerah kami serta beralamat di atas, orang tersebut berkelakuan baik dan  belum pernah tersangkut perkara polisi dan surat keterangan ini diberikan kepada yang bersangkutan untuk surat pengantar pengajuan SKKB.").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("12").create()
        ));               
        //content main
        ...
        myDoc.addEle(Paragraph.withPieces(
                ParagraphPiece.with("\t").withStyle().font(Font.TIMES_NEW_ROMAN_BOLD).fontSize("12").create(),
                ParagraphPiece.with("Demikian surat keterangan ini dibuat dan untuk dipergunakan sebagaimana mestinya.").withStyle().font(Font.TIMES_NEW_ROMAN).fontSize("12").create()
        ));               
